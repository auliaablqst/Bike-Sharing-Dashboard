...
col3.metric("👤 Pengguna Terdaftar", f"{int(filtered_day['registered'].sum()):,}")
col4.metric("🚶 Pengguna Kasual",    f"{int(filtered_day['casual'].sum()):,}")
st.markdown("---")
...
with tab1:
    if filtered_day.empty:
        ...
        season_data = (
            filtered_day.groupby('season')['cnt'].mean()
            .reindex([s for s in SEASON_ORDER if s in filtered_day['season'].unique()])
        )
        fig, ax = plt.subplots(figsize=(8, 5))
        colors = [PALETTE_S[SEASON_ORDER.index(s)] for s in season_data.index]
        bars = ax.bar(season_data.index, season_data.values,
...
with tab2:
    if filtered_day.empty:
        ...
        weather_data = (
            filtered_day.groupby('weathersit')['cnt'].mean()
            .reindex([w for w in WEATHER_ORDER if w in filtered_day['weathersit'].unique()])
        )
        fig, ax = plt.subplots(figsize=(8, 5))
        colors_w = [PALETTE_W[WEATHER_ORDER.index(w)] for w in weather_data.index]
        bars = ax.bar(weather_data.index, weather_data.values,
                      color=colors_w, edgecolor='white', linewidth=1.2)
        for bar in bars:
            ax.text(bar.get_x() + bar.get_width()/2,
                    bar.get_height() + 50,
                    f'{bar.get_height():,.0f}',
...
with col_a:
    ...
    st.markdown("""
    1. **Fall + Clear** = kondisi paling produktif. Cuaca buruk menurunkan permintaan hingga **~63%**.
    2. Pengguna **registered** mendominasi hari kerja dengan pola rush hour pagi & sore.
       Pengguna **casual** & akhir pekan: puncak di siang hari (rekreasional).
    """)
...
with col_b:
    ...
    2. 🕗 Isi ulang stasiun perkantoran sebelum jam **08.00 & 16.30** (hari kerja).
